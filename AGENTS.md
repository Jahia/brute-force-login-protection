...
- **artifactId**: `brute-force-login-protection` | **version**: `2.0.1-SNAPSHOT`
...
| `BruteForceLoginProtectionConstants` | All string keys + `DEFAULT_TIME_TO_IDLE = 3600` |
...
| Operation | Name |
|-----------|------|
| Query | `bruteForceLoginProtectionSettings` → `{activated, nbFailedLoginMax, whitelistIps, timeToIdle}` |
| Query | `bruteForceLoginProtectionTrackedIps` → `[{ip, nbFailedLogins, blocked}]` |
| Mutation | `bruteForceLoginProtectionSaveSettings(activated, nbFailedLoginMax, whitelistIps, timeToIdle?)` → Boolean |
