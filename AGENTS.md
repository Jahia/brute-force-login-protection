...
- **The whitelist wins over an existing ban.** `isIpCurrentlyBanned` returns `false` for a whitelisted IP, so adding an address to `whitelist_ips` immediately lifts enforcement of a ban it already accrued (the ban record itself stays until it expires). This matches `BlocklistService`. Consequence: `banIp` on a whitelisted address is recorded but never enforced. Bans are keyed on **IP, not username**, so with the default `trust_x_forwarded_for=false` a single ban can cover everyone behind a reverse proxy — the whitelist is the escape hatch, and the admin UI offering `unbanIp` sits behind the same valve.
...
- **`ignore_paths` vs `ignore_patterns`.** Two distinct exemptions: `ignore_patterns` is a list of **regexes matched (anchored, full-match, lower-cased) against the username** (ReDoS-guarded via a bounded 50ms interruptible executor). When a match **cannot be evaluated** — timeout, saturated pool, or executor torn down — the pattern is treated as **not matched** and the failure is **counted**, never silently exempted (ADR 0006 / GHSA-7qgr-2hqv-r344). Patterns are linted by `RegexSafetyCheck` on save *and* on `.cfg` load (unsafe entries dropped with a WARN); that lint is best-effort and provably incomplete, so the runtime disposition is the actual control. `ignore_paths` is a list of **literal case-sensitive substrings matched against `request.getRequestURI()`** — *not* regex, checked in `AuthValveFailureSource.invoke()` before the detector chain runs. Use `ignore_paths` when a non-login endpoint is falsely triggering a jail because a client sends a stale/wrong `Authorization` header on it (the built-in Basic/APIToken/jahiatoken detectors fire on header presence regardless of URL, since the valve is at position 0). Both only skip failure *detection*; ban *enforcement* is unaffected.
